...
fractions
"""
def extreme(Y,Kmin,Kmax,Wmin,Wmax,NPASSES,ethresh,rthresh,tries=10,fudgefactor=1.0,revcomp=True):
    ...
    pos_seqs = Y
    #removed grabbing middle sequences because not merged footprints not centrally enriched
...
def main():
    ...
    description = "The program applies a modified EXTREME algorithm to find short motifs in a FASTA file. This variant is best for finding motifs in footprint data."
    ...
    tries = args.tries
    theta_motifs, theta_background_matrices, lambda_motifs, fractionses, distanceses, logevs, disc_pwms, disc_logevs = extreme(seqs,mink,maxk,minw,maxw,nmotifs,ethresh,rthresh,tries,fudgefactor)
    k = 1
